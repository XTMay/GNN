...
class QM9Dataset:
    """
    QM9数据集处理类
    负责数据下载、预处理、标准化和数据集划分
    """

    def __init__(self, root='./Dataset', target=0, transform=None):
        """
        初始化QM9数据集

        Args:
            root: 数据存储路径
            target: 预测目标索引 (0-18, 对应不同的分子属性)
            transform: 数据变换
        """
        self.root = root
        self.target = target
        ...
        self.target_names = [
            'dipole_moment',      # 0: 偶极矩
            'isotropic_polarizability',  # 1: 各向同性极化率
            'homo',               # 2: HOMO能级
            'lumo',               # 3: LUMO能级
            'gap',                # 4: HOMO-LUMO能隙
            'electronic_spatial_extent',  # 5: 电子空间扩展
            'zero_point_vibrational_energy',  # 6: 零点振动能
            'internal_energy_0K',  # 7: 0K内能
            'internal_energy_298K',  # 8: 298K内能
            'enthalpy_298K',      # 9: 298K焓
            'free_energy_298K',   # 10: 298K自由能
            'heat_capacity',      # 11: 热容
            'atomization_energy', # 12: 原子化能
            'atomization_enthalpy',  # 13: 原子化焓
            'atomization_free_energy',  # 14: 原子化自由能
            'rotational_constant_A',  # 15: 转动常数A
            'rotational_constant_B',  # 16: 转动常数B
            'rotational_constant_C',  # 17: 转动常数C
            'vibrational_frequencies'  # 18: 振动频率
        ]

        print(f"正在加载QM9数据集，目标属性: {self.target_names[target]}")
    ...
    def load_data(self):
        ...
        Returns:
            ...
            num_features: 节点特征维度
            target_std: 目标值标准差（用于反标准化）
            target_mean: 目标值均值（用于反标准化）
        """

        # 下载QM9数据集
        print("下载QM9数据集...")
        dataset = QM9(root=self.root, transform=self.transform)

        # 移除不稳定的分子（通常前几个样本）
        dataset = dataset[3000:]  # QM9数据集中前3000个样本通常有问题
        ...
        targets = []
        for data in dataset:
            targets.append(data.y[0, self.target].item())
        ...
        # 标准化目标值
        for data in dataset:
            target_value = (data.y[0, self.target].item() - target_mean) / target_std
        ...
        return train_loader, val_loader, test_loader, dataset[0].x.shape[1], target_std, target_mean
...
def plot_training_history(train_losses, val_losses):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('/Users/xiaotingzhou/Downloads/GNN/training_history.png')
    plt.show()
...
def plot_predictions(true_values, predictions, target_name):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('/Users/xiaotingzhou/Downloads/GNN/predictions_comparison.png')
    plt.show()
...
def main():
    ...
    print("=" * 50)
    print("开始数据加载和预处理...")
    dataset_handler = QM9Dataset(root='/Users/xiaotingzhou/Downloads/GNN/Dataset',
                                target=target_index,
                                transform=transform)

    train_loader, val_loader, test_loader, num_features, target_std, target_mean = dataset_handler.load_data()
    target_name = dataset_handler.target_names[target_index]
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            patience_counter = 0
            # 保存最佳模型
            torch.save(model.state_dict(), '/Users/xiaotingzhou/Downloads/GNN/best_model.pth')
        else:
            patience_counter += 1
    ...
    # 加载最佳模型
    model.load_state_dict(torch.load('/Users/xiaotingzhou/Downloads/GNN/best_model.pth'))
    ...
    np.save('/Users/xiaotingzhou/Downloads/GNN/results.npy', results)
    print("结果已保存到 results.npy")
